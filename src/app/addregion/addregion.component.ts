...
/* countries inside seperate array */
  ...
  osandnzcountries(){
    ...
    .then(function (response) {
      ...
      for(var i = 0; i < response.data.length; i++) {
          ...
          obj['item_id'] = i+1;
          obj['item_text'] = response.data[i].common;
          self.ozandnzcountry.push(obj);
    } 
      console.log(self.ozandnzcountry);
    })
    .catch(function (error) {
      console.log('An Error occured',  error);
  ...
  addregion(){
    var self = this;
    console.log(self.regionname);
    console.log(self.cntries);
    for( var i = 0;i<self.cntries.length;i++){
      self.cntryarray.push(self.cntries[i].item_text);
    }
    console.log(self.cntryarray);
    var costring = JSON.stringify(self.cntryarray);
    axios.post('http://103.214.233.141:3333/v1/secure/ingram/regions',{
